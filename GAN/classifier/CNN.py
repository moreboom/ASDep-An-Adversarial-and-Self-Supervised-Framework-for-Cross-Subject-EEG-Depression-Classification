...
parser.add_argument("--n_epochs", type=int, default=100, help="number of epochs of training")  # 500 is ok
...
def cnn(sub_index, data_train, label_train, data_test, label_test):
    ...
    classifier = nn.DataParallel(classifier, device_ids=[0, 1, 2])
    ...
    for epoch in range(opt.n_epochs):
        for step, data in enumerate(dataloader):
            ...
            if step % 1 == 0:
                ...
                accuracy = float((pred == label_test).cpu().numpy().astype(int).sum()) / float(label_test.size(0))
